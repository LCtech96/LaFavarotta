...
export function PostsFeed() {
  ...
  useEffect(() => {
    // Load profile image from database with localStorage fallback
    const loadProfileImage = async () => {
      try {
        // Prima prova a caricare dal database
        const response = await fetch('/api/images/general?type=profile')
        if (response.ok) {
          const data = await response.json()
          if (data.imageUrl) {
            const savedProfile = data.imageUrl
    ...
    const loadPosts = async () => {
      try {
        ...
        const response = await fetch('/api/posts', {
          cache: 'no-store', // Forza il fetch sempre aggiornato
        })
        ...
        if (response.ok) {
          ...
          // Se la risposta non è ok, logga l'errore
          const errorData = await response.json().catch(() => ({}))
          console.error('Error loading posts from database:', response.status, errorData)
        }
      } catch (error) {
    ...
    // Reload posts periodically to sync with database
